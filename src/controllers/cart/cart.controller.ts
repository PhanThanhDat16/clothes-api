...
export const cartController = {
  addProductInCart: async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(HttpStatus.OK).json({ message: 'Item added to cart', newCartItem })
      return
    } catch (error) {
  ...
  getItemsInCartByUserId: async (req: Request, res: Response) => {
    const userId = req.params.id
    try {
      const existingCart = await cartService.findOne(userId)
      if (!existingCart) {
  ...
  updateProductInCart: async (req: Request, res: Response) => {
    const data = req.body
    const userId = req.params.id
    const { itemId, size, quantity } = data
    try {
